...
const cmdToPoints = (start, cmd) => {
  const num = parseInt(cmd.slice(1));
  const points = [];
  if (cmd[0] === 'R') {
    for (let i = 0; i < num; i++) {
      points.push([start[0] + i + 1, start[1]]);
    }
  }
  if (cmd[0] === 'L') {
    for (let i = 0; i < num; i++) {
  ...
  if (cmd[0] === 'U') {
    for (let i = 0; i < num; i++) {
      points.push([start[0], start[1] + i + 1]);
    }
  }
  if (cmd[0] === 'D') {
    for (let i = 0; i < num; i++) {
...
const parsePointsFromInput = (input) => {
  ...
  input.split(',').forEach(cmd => {
    ...
    points = points.concat(cmdToPoints(lastPoint, cmd));
  })
  return points;
}

const hashPoint = (point) => `${point[0]}-${point[1]}`

const manhattanDist = (point) => Math.abs(point[0]) + Math.abs(point[1]);

const getIntersections = (inputs) => {
  const inputPoints = inputs.map(parsePointsFromInput)
  
  const input1Points = new Set(inputPoints[1].map(hashPoint));
  const intersections = inputPoints[0].filter(point => input1Points.has(hashPoint(point))).filter(point => point[0] !== 0 || point[1] !== 0);
  return intersections;
}

const getClosestIntersectionManhattanDist = (inputs) => {
  const intersections = getIntersections(inputs);
  let closestIntersection = intersections[0];
  let closestDist = manhattanDist(closestIntersection);
  intersections.slice(1).forEach(intersection => {
    const dist = manhattanDist(intersection);
    if (dist < closestDist) {
      closestDist = dist;
...
// question 1
console.log(getClosestIntersectionManhattanDist(test1));
console.log(getClosestIntersectionManhattanDist(test2));
console.log(getClosestIntersectionManhattanDist(mainInputs));


const getHashPointToGridDist = (input) => {
  const inputPoints = parsePointsFromInput(input)
  const gridDistMap = {};
  for (let i = 0; i < inputPoints.length; i++) {
    const hash = hashPoint(inputPoints[i]);
    if (!gridDistMap[hash]) {
      gridDistMap[hash] = i;
    }
  }
...
const getClosestIntersectionGridDist = (inputs) => {
  const gridDistMaps = inputs.map(getHashPointToGridDist);

  const getGridDist = (point) => {
    const hash = hashPoint(point);
    return gridDistMaps[0][hash] + gridDistMaps[1][hash];
  }

  const intersections = getIntersections(inputs);
  let closestIntersection = intersections[0];
  let closestDist = getGridDist(closestIntersection);
  intersections.slice(1).forEach(intersection => {
    const dist = getGridDist(intersection);
    if (dist < closestDist) {
      closestDist = dist;
      closestIntersection = intersection;
    }
...
// question 2
console.log(getClosestIntersectionGridDist(test1));
console.log(getClosestIntersectionGridDist(test2));
console.log(getClosestIntersectionGridDist(mainInputs));